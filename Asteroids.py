...
def show_go_screen():
    ...
    draw_text(screen, "A D keys move, Space to fire", 22,
              WIDTH / 2, HEIGHT / 2)
...
while running:
    ...
    if player.lives == 0 and not death_explosion.alive():
        ...
        cur = con.cursor()
        sql = "INSERT INTO score1 (schet) VALUES(?)"
        cur.execute(sql, (basa,))
        con.commit()
        s = cur.execute("SELECT MAX(schet) FROM score1").fetchall()
        con.commit()
        spisok = ([x[0] for x in s])
        tex = "".join(str(x) for x in spisok)
        f = open('best.txt', 'w')
        f.writelines(('BEST SCORE: ', tex))
        f.close()
        game_over = True
